extends Node2D

const TILE_SIZE := 16
const MAP_W := 10
const MAP_H := 12

var player: Node2D
var quest_state: QuestState
var dialogue_manager: CanvasLayer
var trinket_received: bool = false
var tilemap: TileMapLayer
var miller_npc: Node2D


func _ready() -> void:
	quest_state = SceneManager.quest_state
	quest_state.set_state(QuestState.State.AT_HOMESTEAD)
	dialogue_manager = SceneManager.get_dialogue_manager()
	_create_tilemap()
	_setup_npc()
	_setup_player()


func _create_tilemap() -> void:
	tilemap = TileMapLayer.new()
	tilemap.name = "TileMap"
	add_child(tilemap)

	var tileset := TileSet.new()
	tileset.tile_size = Vector2i(TILE_SIZE, TILE_SIZE)
	tileset.add_custom_data_layer()
	tileset.set_custom_data_layer_name(0, "walkable")
	tileset.set_custom_data_layer_type(0, TYPE_BOOL)

	var source := TileSetAtlasSource.new()
	source.texture = _create_tile_texture()
	source.texture_region_size = Vector2i(TILE_SIZE, TILE_SIZE)

	for i in range(4):
		source.create_tile(Vector2i(i, 0))
		var tile_data := source.get_tile_data(Vector2i(i, 0), 0)
		tile_data.set_custom_data_by_layer_id(0, i != 2 and i != 3)

	var source_id := tileset.add_source(source)
	tilemap.tile_set = tileset

	for x in range(MAP_W):
		for y in range(MAP_H):
			tilemap.set_cell(Vector2i(x, y), source_id, Vector2i(0, 0))  # grass

	# Path
	for y in range(MAP_H):
		tilemap.set_cell(Vector2i(5, y), source_id, Vector2i(1, 0))

	# Homestead building
	for x in range(4, 7):
		for y in range(2, 4):
			tilemap.set_cell(Vector2i(x, y), source_id, Vector2i(2, 0))  # building
	tilemap.set_cell(Vector2i(5, 4), source_id, Vector2i(1, 0))  # door area

	# Trees
	for y in range(MAP_H):
		tilemap.set_cell(Vector2i(0, y), source_id, Vector2i(3, 0))
		tilemap.set_cell(Vector2i(MAP_W - 1, y), source_id, Vector2i(3, 0))


func _create_tile_texture() -> ImageTexture:
	var img := Image.create(TILE_SIZE * 4, TILE_SIZE, false, Image.FORMAT_RGB8)
	var colors := [Color(0.3, 0.6, 0.2), Color(0.7, 0.6, 0.4), Color(0.5, 0.3, 0.2), Color(0.1, 0.4, 0.1)]
	for i in range(4):
		for x in range(TILE_SIZE):
			for y in range(TILE_SIZE):
				img.set_pixel(i * TILE_SIZE + x, y, colors[i])
	return ImageTexture.create_from_image(img)


func _setup_npc() -> void:
	miller_npc = Node2D.new()
	miller_npc.set_script(load("res://scripts/overworld/npc.gd"))
	miller_npc.npc_name = "Miller"
	miller_npc.trigger_mode = "interact"
	miller_npc.npc_color = Color(0.7, 0.6, 0.4)
	add_child(miller_npc)
	miller_npc.setup(Vector2i(5, 5))
	miller_npc.set_dialogue(func(_s: int) -> Array: return DialogueData.homestead_npc())


func _setup_player() -> void:
	player = Node2D.new()
	player.set_script(load("res://scripts/overworld/player.gd"))
	add_child(player)
	var blockers: Array[Vector2i] = [miller_npc.grid_pos]
	player.setup(Vector2i(5, 10), tilemap, blockers)
	player.interacted.connect(_on_interact)
	player.moved.connect(_on_player_moved)

	var camera := Camera2D.new()
	camera.zoom = Vector2(3, 3)
	player.add_child(camera)


func _on_interact(facing_pos: Vector2i) -> void:
	if dialogue_manager.is_active():
		return
	if facing_pos == miller_npc.grid_pos and not trinket_received:
		trinket_received = true
		player.disable_input()
		dialogue_manager.play(DialogueData.homestead_npc())
		await dialogue_manager.dialogue_finished
		player.enable_input()


func _on_player_moved(new_pos: Vector2i) -> void:
	if dialogue_manager.is_active():
		return

	if trinket_received and new_pos.y >= MAP_H - 1:
		quest_state.set_state(QuestState.State.RETURNING)
		player.disable_input()
		dialogue_manager.play(DialogueData.smoke_on_horizon())
		await dialogue_manager.dialogue_finished
		SceneManager.transition_to("res://scenes/overworld/village_destroyed.tscn")
